...
class detectPage(QWidget):
    def __init__(self, nombre: str, time: str):
        super().__init__()
        self.nombre = nombre
        self.time = time
        self.resize(1440,1024)
...
class loadPage(QWidget):
    ...
    def openDetected(self,nombre,time):
        self.ui = detectPage(nombre,time)
        self.hide()
        self.ui.show()
    ...
    def onFinished(self):
        ...
        if(self.myLongTask.nombre != -1):
            self.openDetected(self.myLongTask.nombre, self.elapsed_time)
        else:
